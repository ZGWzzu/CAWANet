...
class VOCSegmentation(data.Dataset):
    def __init__(self, voc_root, year="2012", transforms=None, txt_name: str = "train.txt",train1 = False):
        ...
        root = os.path.join(voc_root, "AKA")
    ...
    def __getitem__(self, index):
        ...
        if scale_ratio != None:
            ...
            target = target.resize((int(w*scale_ratio),int(h*scale_ratio)),Image.BILINEAR)
            edg_mask = edg_mask.resize((int(w*scale_ratio),int(h*scale_ratio)),Image.BILINEAR)
        ...
        return img, target,cs_label,1,self.images[index]
